...
recordRoutes.route('/swarms').post(function (req, res) {
  ...
  dbConnect.collection('swarms').insertOne(swarmDoc, function (err) {
    if (err) {
...
recordRoutes.route('/swarms/deactivate').put(function (req, res) {
  ...
  dbConnect
    ...
    .updateOne(
      swarmDoc,
      { $set: { activeSwarm: false, deactivated: new Date() } },
      function (err) {
        if (err) {
...
recordRoutes.route('/swarms').delete((req, res) => {
  ...
  dbConnect.collection('swarms').deleteOne(swarmQuery, function (err, _result) {
    if (err) {
      res
        .status(400)
